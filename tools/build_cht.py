...
輸出:
  ...
  - pq3_big5.fnt:Big5 點陣字型,格式對齊 ScummVM Graphics::Big5Font::loadPrefixedRaw:
...
# 引擎硬寫的 UI 字串(狀態列/暫停/道具欄/存讀檔)不在 translation.tsv 裡,字集只從譯文
# 收就會漏掉只出現在那些字串裡的字(例如「擇」),實機顯示成空白。這裡一律補烘。
# 內容 = patches/0001-agi-cht-zh_twn.patch 的 systemui.cpp 硬寫 Big5 字串解碼後的字集。
ENGINE_UI_CHARS = (
    "。上中也任位你來個停入分到前即取可向回如始存將帶得怎想意戲按描擇新方時暫"
    "有案檔欄沒消為玩生用發的目空繼續聲自著要該誤身載述這遊選重錄錯鍵開關離音麼，：？"
)
