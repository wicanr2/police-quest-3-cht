...
def _flip_h(bm, w, h):
    """水平翻轉 index bitmap（mirrorBits 標記的 loop 用）。"""
    out = bytearray(len(bm))
    for y in range(h):
        row = bm[y*w:(y+1)*w]
        out[y*w:(y+1)*w] = row[::-1]
    return out
...
class Cel:
    # mirrored：這個 cel 屬於 mirrorBits 標記的 loop。bitmap 存的是「引擎畫出來的樣子」
    # （已水平翻轉），所以 encode 時必須翻回去，否則引擎會再翻一次 → 雙重翻轉。
    __slots__ = ('w', 'h', 'dx', 'dy', 'clear', 'bitmap', 'mirrored')
...
class SCI1View:
    def __init__(self, raw):
        # patch header = [type][headerSkip] + headerSkip bytes。PQ3 有 331 支 headerSkip=0
        # （所以「固定跳 2」在絕大多數檔案上看起來是對的），但 view.594 是 32 —— 寫死 2
        # 會從資料中間開始解，症狀是 IndexError 而不是畫面錯，很容易被當成別的問題。
        d = raw[PATCH_HEADER + raw[1]:] if raw[:1] == b'\x80' else raw
        self.data = d
    ...
    def _read_loop(self, loff, ln):
        ...
        mirrored = bool(self.mirror_bits >> ln & 1)
        cels = []
        for cn in range(cel_count):
            coff = u16(d, loff + 4 + cn * 2)
            c = self._read_cel(coff)
            if mirrored:
                # mirrorBits 標記的 loop 與另一個 loop 共用 cel 資料，引擎繪製時水平翻轉
                # （view.cpp kViewVga：mirrorFlag + displaceX 取負）。不翻的話 decode 出來
                # 的是未翻轉的來源 loop，逐像素比對會有約半數不符——看起來像 codec 解錯，
                # 其實只是少做這一步。
                c.bitmap = _flip_h(c.bitmap, c.w, c.h)
                c.dx = -c.dx
                c.mirrored = True
            cels.append(c)
        return cels
...
def encode_uncompressed(v, replacements=None):
    ...
    for li, cels in enumerate(v.loops):
        for ci, c in enumerate(cels):
            ...
            out[co+4] = (-c.dx if c.mirrored else c.dx) & 0xFF  # 讀取時已取負，寫回要還原
            out[co+5] = c.dy
            out[co+6] = c.clear
            out[co+7] = 0
            bmp = replacements.get((li, ci), c.bitmap)
            if c.mirrored:
                # bitmap/replacement 都是「畫出來的樣子」；資源裡存的是未翻轉版本，
                # 引擎繪製時才翻。這裡不翻回去就會雙重翻轉。
                bmp = _flip_h(bmp, c.w, c.h)
